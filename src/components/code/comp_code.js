...
export class COMPCode extends Component {
  ...
  ValidateCode = async (e) => {
    ...
    if (e.target.value === "" || re.test(e.target.value)) {
      ...
      let pin = `${document.getElementById("A0").value}${
        ...
        <Container className="p-3">
          <Row>
            ...
            <Col xs={12} className="pt-4 text-center">
              {!this.state.intervalValid && !this.state.changeInputTextError ? (
                <label style={label_(2, 0, "0.8em")} onClick={this.NewCode}>
              ...
              {this.state.changeInputTextError ? (
                <label style={label_(7, 0, "0.8em")}>Código incorrecto</label>
              ) : (
                ""
              )}
            </Col>
